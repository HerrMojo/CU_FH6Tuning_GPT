/* Forza Tune App - tuning calculations */
(function () {
  ...
  function buildTires(ctx) {
    const { raceType, surfacePreset, compoundPreset, drivetrain, weightDistribution, classFactor, horsepower } = ctx;
    let front = 28.5;
    let rear = 28.0;
    ...
    return {
      ...
      note: raceType === 'drag'
        ? 'Drag setup uses high front pressure and low drive-tire pressure for launch bite.'
        : 'Check telemetry after a run; warm tire pressure is the real judge.'
    };
  }

  function buildAlignment(ctx) {
    const { raceType, surfacePreset, classFactor } = ctx;
    let frontCamber;
    let rearCamber;
    let frontToe = 0;
    let rearToe = 0;
  ...
  function buildSuspension(ctx) {
    const { raceType, surfacePreset, classFactor, weightDistribution, drivetrain, weight } = ctx;
    const frontWeightBias = FT.clamp((weightDistribution - 50) / 25, -0.3, 0.5);
    let springBase = FT.lerp(0.33, 0.72, classFactor) - surfacePreset.softness * 0.25;
    let arbBase = FT.lerp(0.38, 0.72, classFactor) - surfacePreset.softness * 0.18;
    ...
    const reboundBase = FT.clamp(springBase + 0.16, 0.18, 0.9);
    const bumpBase = FT.clamp(reboundBase * 0.62, 0.12, 0.65);
    const frontReboundRatio = FT.clamp(reboundBase + frontWeightBias * 0.2, 0.15, 0.95);
    const rearReboundRatio = FT.clamp(reboundBase - frontWeightBias * 0.1, 0.15, 0.95);
    const frontBumpRatio = FT.clamp(bumpBase + frontWeightBias * 0.08, 0.1, 0.7);
    const rearBumpRatio = FT.clamp(bumpBase - frontWeightBias * 0.04, 0.1, 0.7);

    function dampingSetting(ratio) {
      return FT.round(FT.clamp(1 + ratio * 19, 1, 20), 1);
    }

    const lowRideHeightIn = FT.lerp(5.4, 3.7, classFactor) + surfacePreset.softness * 0.6;
    ...
    return {
      ...
      damping: {
        frontReboundSetting: dampingSetting(frontReboundRatio),
        rearReboundSetting: dampingSetting(rearReboundRatio),
        frontBumpSetting: dampingSetting(frontBumpRatio),
        rearBumpSetting: dampingSetting(rearBumpRatio),
        ...
        note: "Damping is shown on Forza's 1–20 scale. Keep bump lower than rebound so the car absorbs curbs and dirt cuts instead of pogo-sticking."
      },
      note: raceType === 'drag'
  ...
  function buildDiff(ctx) {
    const { raceType, surfacePreset, drivetrain, classFactor, horsepower } = ctx;
    ...
    if (drivetrain === 'FWD') {
      if (raceType === 'drag') {
        ...
        frontAccel = 45 + classFactor * 18 - surfacePreset.softness * 10;
    ...
    if (drivetrain === 'RWD') {
      if (raceType === 'drag') {
        ...
        rearAccel = 50 + classFactor * 22 + powerAdd * 30 - surfacePreset.softness * 8;
        rearDecel = 12 + classFactor * 13 - surfacePreset.softness * 5;
      }
    }
    ...
    if (drivetrain === 'AWD') {
      if (raceType === 'drag') {
        ...
        frontAccel = 18 + classFactor * 8;
        frontDecel = 4 + classFactor * 3;
        rearAccel = 55 + classFactor * 16 - surfacePreset.softness * 7;
        rearDecel = 14 + classFactor * 10 - surfacePreset.softness * 4;
        centerBalanceRear = 60 + classFactor * 8 - surfacePreset.softness * 4;
      }
    }
    ...
    return {
      ...
      note: raceType === 'drift'
        ...
        : raceType === 'drag'
          ...
          : 'Moderate accel lock gives drive without making corner exit feel welded shut.'
    };
  }

  function buildAeroAndBrakes(ctx) {
    const { raceType, surfacePreset, classFactor, weightDistribution, adjustableAero } = ctx;
    let frontAero = null;
    let rearAero = null;
    ...
    if (adjustableAero) {
      if (raceType === 'drag') {
        ...
        frontAero = 0.18 + classFactor * 0.1;
        rearAero = 0.12 + classFactor * 0.08;
      } else {
        frontAero = 0.38 + classFactor * 0.23 - surfacePreset.softness * 0.08;
        rearAero = 0.44 + classFactor * 0.25 - surfacePreset.softness * 0.08;
      }
    }
    ...
    return {
      aero: adjustableAero
        ? {
            frontRatio: FT.round(FT.clamp(frontAero, 0, 1), 2),
            rearRatio: FT.round(FT.clamp(rearAero, 0, 1), 2),
            note: raceType === 'drag' ? 'Minimize aero for straight-line speed.' : 'Use more rear aero if high-speed exits feel nervous.'
          }
        : {
            frontRatio: null,
            rearRatio: null,
            note: 'Aero marked non-adjustable, so no wing slider recommendation is shown.'
          },
      brakes: {
        balanceFrontPercent: FT.round(FT.clamp(brakeBalanceFront, 45, 56), 1),
  ...
  function generateTune(options) {
    ...
    const weightDistribution = FT.clamp(parseFloat(options.weightDistribution) || 52, 35, 70);
    ...
    const adjustableAero = !!options.adjustableAero;
  ...
  const CORRECTIONS = {
    understeerEntry: {
      ...
      changes: [
        ...
        'For AWD, reduce front decel diff slightly.'
      ],
    },
    understeerThrottle: {
      label: 'Understeers on throttle',
      changes: [
        'Reduce rear diff accel by 5% to 10%.',
        'For AWD, move center balance 3% to 6% rearward.',
        'Soften front springs by 5% to 10% or lower front ARB 2 to 5 points.',
        'If front tires overheat, raise front pressure 0.3 PSI and retest.'
      ],
    },
    oversteerEntry: {
      label: 'Oversteers on entry',
      changes: [
        'Lower rear anti-roll bar by 2 to 7 points.',
        'Increase rear diff decel by 3% to 8% for more stability.',
        'Reduce rear brake pressure feel by moving brake balance 1% frontward.',
        'Add 0.1° to 0.2° more rear camber if rear temps are even.'
      ],
    },
    powerOversteer: {
    ...
    bouncingOffroad: {
      label: 'Bouncing offroad',
      changes: [
        'Raise ride height 5% to 10%.',
        'Soften springs 5% to 12%.',
        'Lower rebound and bump damping, keeping bump around 55% to 65% of rebound.',
        'Lower tire pressure 1 to 2 PSI if the car skates across bumps.'
      ],
    },
    limiterEarly: {
